locals {
  ...
  max_subnet_length = max(
    ...
    local.len_transit_gateway_subnets,
  )
...
resource "aws_route" "public_additional" {
  count = local.create_vpc && var.enable_additional_public_route ? 1 : 0
...
locals {
  create_transit_gateway_subnets = local.create_vpc && local.len_transit_gateway_subnets > 0
}

resource "aws_subnet" "transit_gateway" {
  count = local.create_transit_gateway_subnets && (!var.one_nat_gateway_per_az || local.len_transit_gateway_subnets >= length(var.azs)) ? local.len_transit_gateway_subnets : 0
  ...
  tags = merge(
    {
      Name = try(
        var.transit_gateway_subnet_names[count.index],
        format("${var.name}-${var.transit_gateway_subnet_suffix}-%s", element(var.azs, count.index))
      )
    },
    var.tags,
    var.transit_gateway_subnet_tags,
    lookup(var.transit_gateway_subnet_tags_per_az, element(var.azs, count.index), {})
  )
}

resource "aws_route_table" "transit_gateway" {
  count = var.separate_tgw_route_table_per_az ? length(var.azs) : local.create_transit_gateway_subnets ? 1 : 0

  vpc_id = local.vpc_id

  tags = merge(
    { "Name" = var.separate_tgw_route_table_per_az ? "${var.name}-transit_gateway_${var.azs[count.index]}" : "${var.name}-transit_gateway" },
    var.tags,
...
resource "aws_route_table_association" "transit_gateway" {
  ...
  route_table_id = var.separate_tgw_route_table_per_az ? aws_route_table.transit_gateway[count.index].id : aws_route_table.transit_gateway[0].id
}

resource "aws_route" "transit_gateway_to_firewall" {
  count = local.create_vpc && var.enable_transit_gateway_to_firewall_route ? local.len_transit_gateway_subnets : 0
